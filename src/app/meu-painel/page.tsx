  ...
  return (
    <div className="rounded-xl border border-zinc-800 bg-zinc-900 px-6 py-5 grid grid-cols-[auto_1fr_auto] gap-x-8 items-center">
      ...
      <div className="flex flex-col gap-1 border-l border-zinc-800 pl-8">
        <div className="text-[42px] font-extrabold leading-none tracking-tight text-[#FB923C]">
  ...
  return (
    <div className="flex items-center gap-2">
      <button type="button" onClick={() => handle(false)} disabled={loading}
        ...
        style={{ background: 'rgba(251, 146, 60,0.08)', color: 'var(--brand-primary)', border: '1px solid rgba(251, 146, 60,0.15)' }}>
  ...
  return (
    <div
      ...
      className={`rounded-xl p-5 transition-colors ${
        featured
          ? 'border-2 border-[#FB923C] bg-[rgba(251, 146, 60,0.06)] shadow-[0_0_20px_rgba(251, 146, 60,0.15)]'
          : `border border-zinc-800 bg-zinc-900 ${onClick ? 'cursor-pointer hover:border-zinc-600 hover:bg-zinc-800/60' : ''}`
      }`}
    >
      <div className="flex items-start justify-between mb-3">
        <span className={`text-[11px] font-semibold uppercase tracking-wider ${featured ? 'text-[#FB923C]' : 'text-zinc-500'}`}>{label}</span>
        <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${featured ? 'bg-[rgba(251, 146, 60,0.15)] text-[#FB923C]' : accent}`}>
          <Icon size={14} />
        </div>
      </div>
      <div className={`font-bold tracking-tight break-all leading-tight ${featured ? 'text-2xl text-[#FB923C]' : 'text-lg text-white'}`}>{value}</div>
      {sub && <div className={`text-xs mt-1.5 ${featured ? 'text-[rgba(251, 146, 60,0.6)]' : 'text-zinc-500'}`}>{sub}</div>}
    </div>
  )
}
...
function MiniDonut({ services, expenses }: { services: number; expenses: number }) {
  ...
  return (
    <svg width={52} height={52} viewBox="0 0 52 52" style={{ transform: 'rotate(-90deg)' }}>
      <circle cx={cx} cy={cy} r={r} fill="none" stroke="#27272a" strokeWidth={6} />
      {srvLen > 0 && (
        <circle cx={cx} cy={cy} r={r} fill="none" stroke="#FB923C" strokeWidth={6}
...
function BarChartRow({
  label, minutes, maxMinutes, color = '#FB923C',
}: {
            ...
            {tsByCustomer.length === 0 ? (
              ...
              <div className="space-y-3">
                {tsByCustomer.map((c, i) => {
                  ...
                  const colors = ['#FB923C', '#a78bfa', '#fb923c', '#34d399', '#f472b6', '#60a5fa']
            ...
            <div className="rounded-xl border border-zinc-800 bg-zinc-900 p-5">
              ...
              {tsByProject.length === 0 ? (
                ...
                <div className="space-y-3.5">
                  {tsByProject.map(p => (
                    <BarChartRow key={p.name} label={p.name} minutes={p.minutes} maxMinutes={maxProjectMin} color="#FB923C" />
                  ))}
                </div>
              )}
            </div>
                ...
                <div style={{
                  ...
                  {hoursEntry && (
                    <div style={{ display: 'flex', justifyContent: 'space-between', gap: 20, marginBottom: 5 }}>
                      ...
                      <span style={{ fontSize: 11, fontWeight: 700, color: '#FB923C', fontFamily: 'monospace' }}>
              ...
              <div className="rounded-xl border border-zinc-800 bg-zinc-900 p-5">
                    ...
                    <ComposedChart data={history} margin={{ top: 8, right: hasMonetary ? 48 : 8, left: 0, bottom: 0 }}>
                      ...
                      {avgHours > 0 && (
                        <ReferenceLine
                          ...
                          strokeDasharray="5 4"
                          label={{ value: `~${avgHours.toFixed(0)}h`, position: 'insideTopLeft', fill: 'rgba(251, 146, 60,0.4)', fontSize: 10 }}
                        />
                      )}
                      ...
                      <Bar yAxisId="hours" dataKey="hours" name="Horas" radius={[5, 5, 0, 0]} maxBarSize={36} isAnimationActive>
                        {history.map((p) => (
                          <Cell
                            key={p.ym}
                            fill={p.isCurrent ? '#FB923C' : 'rgba(251, 146, 60,0.22)'}
                          />
                        ))}
                      </Bar>
                ...
                <div className="flex items-center gap-5 mt-5 justify-center flex-wrap">
                  <div className="flex items-center gap-2 text-[11px] text-zinc-500">
                    <div className="w-3 h-3 rounded-sm" style={{ background: 'rgba(251, 146, 60,0.22)' }} />
                    Horas
                  </div>
