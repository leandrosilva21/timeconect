            ...
            {MONTHS_PT.map((m, i) => {
              ...
              return (
                <button key={m} type="button" onClick={() => select(mNum)}
                  ...
                  style={{
                    background: isSelected ? 'var(--brand-primary)' : isToday ? 'rgba(251, 146, 60,0.10)' : undefined,
                    color: isSelected ? '#0A0A0B' : isToday ? 'var(--brand-primary)' : 'var(--brand-text)',
                    border: isToday && !isSelected ? '1px solid rgba(251, 146, 60,0.3)' : '1px solid transparent',
                  }}>
                  {m}
                </button>
              )
